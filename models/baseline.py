...
#Using UNET, which use a Fully Convolutional Network Model for the task
...
def gen(data, au=False):
    while True:
        ...
        yield tf.squeeze(np.array(list_images_aug)), tf.squeeze(np.array(list_gt_aug), axis = 4)
...
if __name__ == '__main__':
    ...
    train_data = list(zip(sorted(glob('../datasets/images/*.jpg')),
                          sorted(glob('../datasets/mask_gt_converted/*.jpg'))))
